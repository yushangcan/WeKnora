...
| CI/Parser | 已有定时和 PR 触发的评测 workflow、缺配置 skipped artifact、当前 Run 自动纳入 comparison、当前 Commit 校验、PR baseline 缺失阻断、质量 gate、Embedding Cache Linux 测试/benchmark workflow、PostgreSQL migration lifecycle workflow；八解析器横评尚未开始 | 评测 workflow 尚未在本项目真实凭据环境运行，且 PR 仍依赖外部服务和 baseline 配置；Parser 仍是可选项目 |
...
- `e97fde17 ci(evaluation): verify current commit and baseline`
