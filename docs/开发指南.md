...
需要验证当前工作区的后端或前端改动时，使用本地 Compose 覆盖配置，避免继续使用远程的 `latest` 镜像：

```bash
# 生成前端静态资源（frontend/Dockerfile 会复制该目录）
sh scripts/build_frontend_dist.sh
# Windows PowerShell 可改用：npm --prefix frontend ci && npm --prefix frontend run build-only

# 使用当前工作区构建 app/frontend，并启动完整依赖
docker compose -f docker-compose.yml -f docker-compose.local.yml build app frontend
docker compose -f docker-compose.yml -f docker-compose.local.yml up -d

# 查看服务状态和后端迁移日志
docker compose -f docker-compose.yml -f docker-compose.local.yml ps
docker compose -f docker-compose.yml -f docker-compose.local.yml logs --tail 200 app
```

`docker-compose.local.yml` 只覆盖 app 和 frontend 的镜像标签（分别为 `weknora-app:local`、`weknora-ui:local`），基础设施、端口、数据卷和依赖关系仍由主 Compose 文件提供。停止服务时不要添加 `-v`，以保留本地知识库和数据库数据。
