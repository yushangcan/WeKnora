                ...
                <t-tooltip :content="item.title" placement="right" :disabled="!uiStore.sidebarCollapsed">
                    <div @click="handleMenuClick(item.path)" @mouseenter="mouseenteMenu(item.path)"
                        ...
                        <div class="menu_item-box">
                            <div class="menu_icon">
                                <img class="icon"
                                    :src="getImgSrc(item.icon == 'zhishiku' ? knowledgeIcon : item.icon == 'agent' ? agentIcon : item.icon == 'organization' ? organizationIcon : item.icon == 'evaluation' ? evaluationIcon : item.icon == 'logout' ? logoutIcon : item.icon == 'setting' ? settingIcon : prefixIcon)"
                                    alt="">
                            </div>
...
const topMenuItems = computed<MenuItem[]>(() => {
    return (visibleMenuArr.value as unknown as MenuItem[]).filter((item: MenuItem) =>
        item.path === 'knowledge-bases' || item.path === 'evaluations' || item.path === 'agents' || item.path === 'organizations' || item.path === 'creatChat'
    );
});

const bottomMenuItems = computed<MenuItem[]>(() => {
    return (visibleMenuArr.value as unknown as MenuItem[]).filter((item: MenuItem) => {
        if (item.path === 'knowledge-bases' || item.path === 'evaluations' || item.path === 'agents' || item.path === 'organizations' || item.path === 'creatChat') {
            return false;
        }
        return true;
    });
...
let evaluationIcon = ref('integration.svg');
...
const getIcon = (path: string) => {
    ...
    const evaluationsActiveState = route.name === 'evaluationHistory';
    ...
    // 评测页复用数据集成图标，以避免引入一套仅用于单个入口的资源。
    evaluationIcon.value = evaluationsActiveState ? 'integration-green.svg' : 'integration.svg';
